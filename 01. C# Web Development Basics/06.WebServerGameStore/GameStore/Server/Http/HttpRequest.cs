namespace GameStore.Server.Http
{
    ...
    using GameStore.Server.Exceptions;
    ...
    public class HttpRequest : IHttpRequest
    {
        ...
        private void ParseQuery(string queryAsString, IDictionary<string, string> dict)
        {
            ...
            {
                string[] queryPairTokens = queryPair.Split(new[] { '=' });
        ...
        {
            if (this.Cookies.ContainsKey(SessionRepository.CookieKey))
            {
                HttpCookie cookie = this.Cookies.GetCookie(SessionRepository.CookieKey);
