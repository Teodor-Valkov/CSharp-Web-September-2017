namespace LearningSystem.Services.Trainer.Implementations
{
    ...
    public class TrainerService : ITrainerService
    {
        ...
        public async Task<IEnumerable<CourseBasicServiceModel>> GetAllListingByTrainerIdAsync(string trainerId, string searchToken, int page)
        {
        ...
        public async Task<bool> IsUserTrainerAsync(int courseId, string trainerId)
        {
            return await this.database
                .Courses
        ...
        public async Task<int> TotalCountAsync(string trainerId, string searchToken)
        {
            if (string.IsNullOrWhiteSpace(searchToken))
            {
                return await this.database.Courses.Where(c => c.TrainerId == trainerId).CountAsync();
            }

            return await this.database
              .Courses
              .Where(c => c.TrainerId == trainerId && c.Name.ToLower().Contains(searchToken.ToLower()))
              .CountAsync();
        }
    }
}